...
globals[contador contadorAct contadorActNull i j tduni1 tduni2 tduni3 tduni4 blacks a m n
         ...
         sum-color4 col npixel z pc nlist nlist1 colorempate2rm
...
[
  ...
  [
    ...
    set nlist1 sentence nlist1 [pcolor] of item z npixel
    ]
    if (pcolor = 8)
    [
    ...
    [
          ...
          ;if sum-color3 > 0 or sum-color4 > 0
          ;[
           ;if(length nlist = 2 or length nlist = 2 or length nlist = 3 or length nlist = 4)[foreach nlist [ask ? [set pcolor one-of nlist1]]];[set pcolor item random 2 [2 8]]]];8]]] ;pc if-else(pc = 2)[set pc 8][set pc 2]]]] ;
           ...
           if sum-color1  = sum-color2 ; if A's = B's then do random selection between {A,B}
            ...
            if sum-color1 != sum-color2 ;If not equal A's and B's then conversion rule
            [
           foreach nlist [ ?1 -> ask ?1 [set pcolor one-of nlist1] ] ;Convert to AB's all 0's and C's in G
   ...
   ;To plotting...
     set a ((count patches with [pcolor = 2]) + (count patches with [pcolor = 8]) + (count patches with [pcolor = white ])
       + (count patches with [pcolor = 4]) + (count patches with [partido = 5]))
...
he model goal is to shows and update rule which statistically has no profit to any
party A,B.
...
@#$#@#$#@
<experiments>
  <experiment name="MenorRemplaincinc0.005rep50" repetitions="300" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>inicio</go>
