...
;npixel is a list of patches pairs with a particular order, length(npixel)=LxL
set Caux npixel

;LxL = N
set N nnodos
    ;patches becomes distributed according to values: persons_party1, persons_party2, and persons_party3
    ;Lists to be filled with the patche distribution per opinioin.
    set tduni1 []
    set tduni2 []
    set tduni3 []
    set tduni4 []

    ;If needed, draw division lines
 ...
 [
   ...
   set npixel replace-item z npixel item (N - 1) npixel ; Put selected patche to list end
 ...
 [
 ;To distribute C nodes, here persons_party3 is less (or more) than #V(R)
 ...
 ;If floors of persons_party1 + persons_party + persons_party3 not equal to network order, and
 ...
 ;Before enter to dynamics, set up some variables
 ...
 ;Variables to count each MC steps
 set contadorAct 0 ;MCs
 set contadorActNull 0 ;MCs before end null states
...
;PROCEDURE-INICIO. Patche Inspection without repetition
;-------------------------------------------------------------------

to inicio

;Cycle repetition: a selection of totaladistribuir nodes with or without repetition, here, instead of follow Redner, we use 600 as the maximum number of repetitions
repeat 1400 ;Ts = stripe formation time
[
   ;Condicion To finish dynamics
  if (tparty_1 = nnodos) or (tparty_2 = nnodos) or (tparty_3 = nnodos) or (tparty_4 = nnodos) or (tparty_5 = nnodos) [stop]

  set N nnodos
  set npixel Caux ; Recover npixel from the previos unmodified setup procedure.
  ...
  [
    ...
    ;Computations over the patche (xi,yj) 4-neighbourhood
    ask item z npixel
    [
    ...
    ;To know the number of cycles after null nodes desapair
    if tparty_4 > 0
      [
        set contadorActNull contadorActNull + 1
      ]

    ;CODES: Party 1 = A color soft black code 2, party 2 = B color gray code 8, party 3 = C color white code white, null = 0 color black code 0
    ...
    ;to enter here, we considering a 4-neighbourhood AB, C excluding C's or only A's or B's
    ...
    ] ;End if, conditions to enter in 4-neighbourhood analisys
        if repeticion = false
...
;Color codes: 45 is white , 55 is 8, 105 is 2, black is 0
...
This is a opinion dynamics model on a finite size toroidal lattice network R. Three active opinions (A,B,C), and possibly a four uncommited inactive voters (0). Opinions
A,B form the coalition AB. We are under assumption of no external influence into the system.

R is a square lattice network with 50,75,100,....,10000 nodes per side, its boundary conditions embbed R on a torus and can be modified by NetLogo by editing the world.

Model shows update rules which statistically led profit to the
huge coalition opinion.
...
Setting the initial conditions, number of coalition nodes (A nodes plus B nodes), third party C nodes, and uncommited nodes O.

Network size N=LxL can be selected with netorder global variable through proper selection of L. 

The model needs: C value (usually fixed to .5#V(R)). C is a real value in [0,1], which indicate the proportion of nodes to be assigned for party C, then we need to indicate opinion A proportion value having in account that B=C-A thus 0 <= A <= C. Once we have fixed C,A we have B (party B proportion).
...
@#$#@#$#@
<experiments>
  <experiment name="MenorRemplaincinc0.005rep50" repetitions="800" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>inicio</go>
    ...
    <steppedValueSet variable="porcentA" first="0.01" step="0.01" last="0.05"/>
